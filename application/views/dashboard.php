...
<section>
                  ...
                  <tr>                 
                    ...
                    <?php foreach ($getPPData as $key => $getPPDataR) {  ?>    
                     <?php if($getPPDataR->program_fee_type=='S') { ?>                
                      <div>                        
                        <input id="a<?= $getPPDataR->id?>" type="radio" value="<?= $getPPDataR->program_name;?>" onchange="getFeeAmt();" class="checkbox1" name="program">
                        <label for="a"><?= $getPPDataR->program_name;?>(₹<?= $getPPDataR->program_fee;?>)</label>
                      </div>
                    <?php } ?>
                      <?php if($getPPDataR->program_fee_type=='I') { ?>
                        <div style="margin-left: 20px;">  
                        <input type="radio" value="<?= $getPPDataR->program_name;?>" class="checkbox1" onchange="getFeeAmt();" name="program">                       
                    ...
                    <?php } ?>  
                      ...
                      <form method="POST" action="<?= site_url('order-pay');?>">
  <input class="textbox"type="hidden" name="TxnRefNo" id="TxnRefNo" value="<?= $TxnRefNo;?>" required />
  <input type="text"  name="Amount" id="Amount" value="" pattern="[1-9]\d*(\.\d+)?" title="Please enter a number greater than or equal to 1" size="40" maxlength="12" required class="form-control col-md-3" placeholder="Enter Amount" style="width:250px;"/> 
  <!-- <div class="tammtp"></div> -->
  <input class="textbox"type="hidden"  name="Currency" id="Currency" value="356" size="50" maxlength="40" required />
  <input class="textbox"type="hidden"  name="MerchantId" id="MerchantId" value="100000000005859" required />
  <input class="textbox"type="hidden"  name="TerminalId" id="TerminalId" value="EG000488" required />
  ...
  <!-- <input class="textbox" type="hidden"  name="FirstName" id="FirstName" value="<?= ucfirst($studentfeeD->name);?>"/> -->
  <input class="textbox" type="hidden"  name="Phone" id="Phone" value="<?= $studentfeeD->mobile?>" size="10" placeholder="1234567890" pattern="[0-9]{10}" maxlength="10"/><hr>
  <div class=""><button type="submit" name="Pay Now" class="btn btn-success">Pay Now</button></div>
</form>                
